...
# case data
f <- system.file("extdata/covid19_daily_reports.csv", package = "covid19clark")
jhu_cases <- read_csv(
  f, col_types = cols(fips = col_character(), admin2 = col_character(),
                      key = col_character())
) %>% filter(country == "US") 
pat <- "Unassigned Location|Grand Princess|Diamond Princess"
jhu_cases <- jhu_cases %>% select(-fips, -country) %>% 
...
# jhu_cases %>% filter(grepl("d.c.", state, ignore.case = TRUE)) %>% view

# fix mismatching states, etc (not thoroughly checked yet)
jhu_fix_state_abbrev <- jhu_cases %>% filter(nchar(state) < 4) %>% 
  rename(state2 = state) %>%
  left_join(., state_names %>% mutate(state2 = tolower(state2))) %>% 
  select(state1, state2, admin2, prov, county, key, x, y, date, cases, deaths)
jhu_full_state_names <- jhu_cases %>% filter(nchar(state) > 3) %>% 
  rename(state1 = state) %>%
  left_join(., state_names) %>%
  mutate(state2 = tolower(state2)) %>% 
  select(state1, state2, admin2, prov, county, key, x, y, date, cases, deaths)  
# jhu_full_state_names %>% filter(state2 == "dc")  

jhu_cases_all <- bind_rows(jhu_fix_state_abbrev, jhu_full_state_names) %>% 
  arrange(date, state1, county) %>% mutate(county = tolower(county))
jhu_cases_all %>% filter(state2 == "ma") %>%
  filter((x == 0 & y == 0) | is.na(x))

# jhu_cases_all %>% filter(state2 == "ma") %>% #filter(is.na(county)) %>% 
#   group_by(date) %>% 
#   summarize(cases = sum(cases))

# state cases
state_cases <- jhu_cases_all %>% 
...
as_tibble(states) %>% select(state, x, y) %>% 
  left_join(state_cases, ., by = c("state1" = "state")) -> state_cases

# state_cases %>% group_by(state1, date) %>% count() %>% filter(n > 1)

# # county cases
# f <- system.file("extdata/covid19_daily.csv", package = "covid19clark")
# daily_cases <- read_csv(f) %>% filter(ctry == "US") %>% 
daily_cases <- jhu_cases_all %>%
  ...
  ungroup() %>% 
  dplyr::select(state1, state2, county, x, y, date, cases, deaths)
```

```{r, echo=FALSE, eval=FALSE}
# checks
daily_cases %>% filter(state2 == "ma") %>% filter(date == max(date)) %>%
  summarize(sum(cases))
daily_cases %>% filter(state2 == "ma") %>%  filter(grepl("worc", county))

# compare against DPH county numbers for 3/24--checks out
daily_cases %>% filter(state2 == "ma") %>% filter((x == 0 & y == 0) | is.na(x))
```
...
st_transform(focal_city, crs = 102008)
...
daily_cases_region <- daily_cases %>% 
  filter(state1 %in% states_in_buffers[[2]] & date == max(date)) %>%
  filter(cases > 0) %>% 
  arrange(state1, date)
...
# x <- daily_cases_region %>% pull(cases)
# size_brks <- as.integer(10^(1:ceiling(log10(max(x)))))
...
p1 <- ggplot(states) + geom_sf() +
  ...
  scale_size_continuous(name = "Cases", limits = c(1, max(size_brks)),
                        breaks = size_brks) +
  coord_sf(xlim = city_box[c(1, 3)] + c(-0.2, 0.2),
           ylim = city_box[c(2, 4)] + c(0.2, -0.2)) +
...
case_rates <- lapply(c(1, 4), function(x) {  # x <- 4
  state_cases %>% filter(state1 %in% !!states_in_buffers[[x]]) %>%
    ...
    dplyr::select(radius, states, date, cases, deaths)
}) %>% do.call(rbind, .)
...
ylimit <- max(case_rates$deaths)
p4 <- state_cases %>%
  filter(state1 %in% states_in_buffers[[2]]) %>%
  filter(date > lubridate::ymd("2020-03-01")) %>%
  ggplot() + geom_line(aes(date, deaths, color = state1)) +
  ylim(0, ylimit) +
  xlab("") + ylab("Deaths") +
