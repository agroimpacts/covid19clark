...
# base <- 5
# nbreaks <- 5
case_breaks <- function(x, base = 5, nbreaks = 5) {
  rng <- c(0.0, max(x) + (base - max(x) %% base))
  brks <- seq(rng[1], rng[2], diff(rng) / 4)
  brks[1] <- 0.01
  return(brks)
}
