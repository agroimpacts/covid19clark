...
# compare against DPH county numbers for 3/31--checks out for state
```
...
as_tibble(daily_cases_region) %>% filter(death_rate < 0.01) %>% 
...
p1 <- ggplot(us_counties) + geom_sf(fill = "grey80", lwd = 0.3) +
  ...
  geom_point(data = as_tibble(daily_cases_region), 
             ...
             shape = 16, stroke = FALSE, color = "yellow") +
  xlab("") + ylab("") + ggtitle("Cases reported by county") +
  scale_size_continuous(name = "Per 1000", 
                        limits = c(0.001, max(size_brks)),
                        breaks = size_brks) +
  # scale_fill_gradient(name = "Cases per 1000", #low = pal[1], high = pal[9],
  #                     breaks = log10(size_brks), 
