...
# daily_cases_region %>% filter(case_rate == max(case_rate))
# daily_cases_region %>% filter(cases == max(cases))


# plotting parameters
size_brks <- case_breaks(daily_cases_region %>% pull(case_rate))  # breaks
...
p1 <- ggplot(states) + geom_sf(fill = "grey50") +
  ...
  #                       breaks = size_brks) +
  scale_fill_continuous(name = "Cases per 1000", palette = reds(100), 
                        limits = c(0, max(size_brks))) +
