...
# states cases
# state_cases <- as_tibble(cases) %>% filter(is.na(county)) %>%
#   dplyr::select(state, date, week, cases) %>%
#   left_join(., states) %>% 
#   st_centroid() %>% as_tibble() %>% 
#   mutate(x = st_coordinates(.)[, 1], y = st_coordinates(.)[, 2]) %>% 
#   dplyr::select(state, date, week, x, y, cases)

# region_cases[[4]] %>% dplyr::filter(state == "massachusetts") %>% 
#   group_by(county) %>% filter(date == max(date)) %>% ungroup() %>% 
#   summarize(sum(cases))
```
...
p1 <- ggplot(states) + geom_sf() + 
  ...
  scale_color_distiller(name = "Date", palette = "RdBu", breaks = my_breaks) +
...
p3 <- ggplot(case_rates) + 
  ...
  scale_color_brewer(name = "", palette = "RdBu") + theme_minimal()
