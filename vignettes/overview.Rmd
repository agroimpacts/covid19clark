...
- Finish final draft of modules and github page
...
df <- read.csv("D:/OLandry/Geospatial Analysis with R/Final Project/R Results/covid19clark/inst/extdata/covid19_daily_reports.csv", stringsAsFactors = FALSE)
...
ggplot(data = world) +
  ...
  scale_colour_manual(NULL, values = c(cases = "red",
                                       deaths = "blue", 
                                       recovered = "purple")) + labs(title="Map of World Showing Cases, Deaths, and Recovered") + theme(plot.title = element_text(hjust = 0.5))
...
ggplot(data = counties) + 
  ...
  scale_colour_manual(NULL, values = c(cases = "red",
                                       deaths = "blue", 
                                       recovered = "purple")) + labs(title="Map of US Showing Cases, Deaths, and Recovered") + theme(plot.title = element_text(hjust = 0.5))
```

*Histograms*
```{r, warning = FALSE, message = FALSE}
#Histogram Showing US Cases, Deaths, and Recovered
ggplot() + geom_histogram(data = US %>% filter(cases > 0),
                          aes(x = x, color = "cases")) + 
  geom_histogram(data = US %>% filter(deaths > 0), 
                 aes(x = x, color = 'deaths')) + 
  geom_histogram(data = US %>% filter(recovered > 0),
                 aes(x = x, color = 'recovered')) + labs(title="Histogram Showing US Cases, Deaths, and Recovered") + theme(plot.title = element_text(hjust = 0.5))

#Histogram Showing Worldwide Cases, Deaths, and Recovered
ggplot() + geom_histogram(data = df %>% filter(cases > 0),
                          aes(x = x, color = "cases")) + 
  geom_histogram(data = df %>% filter(deaths > 0), 
                 aes(x = x, color = 'deaths')) + 
  geom_histogram(data = df %>% filter(recovered > 0),
                 aes(x = x, color = 'recovered')) + labs(title="Histogram Showing Worldwide Cases, Deaths, and Recovered") + theme(plot.title = element_text(hjust = 0.5))

#Histogram Showing US Cases, Deaths, and Recovered Combined
us_counties %>% ggplot() + 
  geom_histogram(aes(x = x, color = state), bins = 15) + labs(title="Histogram Showing US Cases, Deaths, and Recovered Combined") + theme(text = element_text(size=10))

#Histogram Showing US Cases and Deaths
  ggplot() + geom_histogram(data = us_cases_daily$county %>% filter(.$cases > 0),
             aes(x = x, color = "cases")) + 
  geom_histogram(data =us_cases_daily$county %>% filter(.$deaths > 0), 
             aes(x = x, color = 'deaths')) + labs(title="Histogram Showing US Cases and Deaths") +
...
#Barplot Showing Number of COVID-19 Cases by State
 state_cases <- us_cases_daily$state %>% group_by(state1) %>% select(cases) %>% summarise(maxcases = max(cases))
 barplot(state_cases$maxcases, main = "Number of COVID-19 Cases by State", xlab = "US States and Territories", ylab = "Number of Cases", names.arg = c(1:52))
 
#Barplot Showing Number of COVID-19 Deaths by State
  state_deaths <- us_cases_daily$state %>% group_by(state1) %>% select(deaths) %>% summarise(maxdeaths = max(deaths))
  barplot(state_deaths$maxdeaths, main = "Number of COVID-19 Deaths by State", xlab = "US States and Territories", ylab = "Number of Deaths", names.arg = c(1:52))
```
...
- Display Shiny visualizations on Shiny.io web page and Plotly visualizations on Github Pages web space. 
(Zoe and Olivia)
...
- 04/19/20: Zoe finishes rough draft of outputs from Shiny, Olivia finishes rough 
draft of outputs from Plotly.
...
- 04/29/20: Zoe adds the Shiny and Plotly outputs to a Github Pages interface.
...
- Expect to meet the timeline we set for ourselves, with continuing communication 
regarding goals and project milestones.
...
- Create simple Github Pages project and embed data visualization applications in 
the platform.
