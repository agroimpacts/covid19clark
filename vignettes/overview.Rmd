...
```{r, warning = FALSE, message = FALSE}
#Map of Just the U.S. Showing Cases and Deaths 
require(maps)
United_States = map_data("state")
ggplot(data = us_cases_daily$county) + 
  geom_polygon(data = United_States, aes(x = long, y = lat, group = group), fill = 'NA', colour = 'black') + geom_point(data = us_cases_daily$county %>% filter(cases > 0),
             aes(x = x, y = y, color = "cases"), size = 1) + 
  geom_point(data = us_cases_daily$county %>% filter(deaths > 0), 
             aes(x = x, y = y, color = 'deaths'), size = 1) + 
  scale_colour_manual(NULL, values = c(cases = "blue",
                                       deaths = "red")) +
  labs(title="Map of U.S. Showing Cases and Deaths") + 
...
```{r, warning = FALSE, message = FALSE}
#Histogram Showing U.S. Cases and Deaths Combined
us_counties %>% ggplot() + 
  geom_histogram(aes(x = x, fill = state), bins = 15, color = "white") + 
  labs(title="Histogram Showing U.S. Cases and Deaths Combined", 
       x = "Longitude") + theme(text = element_text(size=8.25)) + theme(legend.title = element_blank())
#Histogram Showing U.S. Cases and Deaths Separately 
